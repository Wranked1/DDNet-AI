...
1. Поставь [Node.js](https://nodejs.org) 24 или новее.
2. Скачай **`ddnet-ai.zip`** из [последнего релиза](https://github.com/Wranked1/DDNet-AI/releases/latest) и распакуй в любую папку.
3. Дважды щёлкни **`DDNet AI.vbs`**.

> [!TIP]
> В первый раз окно само скачает Electron (около 160 МБ, с проверкой контрольной суммы) и соберётся в папке `app-win`. Дальше оно открывается сразу и без консоли.
...
<details>
<summary><b>Другие способы запуска</b></summary>
<br>

| Файл | Что делает |
|---|---|
| `run-gui.vbs` | запускает без консоли, бот открывается во вкладке браузера на `http://localhost:7777` |
| `run.bat` | запускает с консолью, в ней же можно писать команды |
| `run-forever.bat` | поднимает бота заново, если он упал или его выкинуло с сервера |
| `run.sh`, `run-forever.sh` | то же для Linux и macOS |
...
Ничего делать не нужно. Бот раз в пять минут проверяет, нет ли новой версии, скачивает её и перезапускается сам, а окно обновляется вместе с ним. Настройки, записи и память карт при этом не трогаются.

> [!NOTE]
> Если бот долго был выключен и хочется обновить его до запуска, дважды щёлкни `update.bat` (на Linux и macOS `update.sh`).
...
| Путь | Что там |
|---|---|
| `settings.json` | сервер, ник, клан, скин |
| `runs/clips` | записи моментов игры |
| `runs/memory` | что бот запомнил о картах |
| `app-win` | собранное окно, его можно удалить, оно соберётся заново |
